...
for zaznam in vysledky:
  seznam = zaznam.values()
  soucet = zaznam[1] + zaznam[2] + zaznam[3] + zaznam[4] + zaznam[5]
  print(soucet)
"""  
  for k,h in zaznam.items():
    if h == int:
      soucet = soucet + h
  print(zaznam)
  print(soucet)

def ohodnot_studenta(a):
  
  print(seznam_znamek)

  B = {vysledky["Český jazyk"], vysledky["Anglický jazyk"], vysledky["Matematika"], vysledky["Biologie"], vysledky["Zeměpis"], vysledky["Dějepis"], vysledky["Ekonomika"], vysledky["Fyzika"], vysledky["Chemie"], vysledky["Informatika"]}
  print B
  a = sum
  print(a)
